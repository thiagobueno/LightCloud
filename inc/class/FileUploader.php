	...
	private function uploadFiles() {
		...
		if ($this->field['input']) {
			...
			if ($validate === true) {
              ...
              // save to database
              $query = Database::instance()->prepare('INSERT INTO files (name, extension, size, public, email, path, temp) VALUES (:name, :extension, :size, 0, :email, :path, :temp)');
              $query->execute([
                'name' => $metas['old_title'],
                'extension' => $metas['extension'],
                'size' => $metas['size'],
